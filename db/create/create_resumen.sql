...
-- Generado a partir de: comercial_desn_db (tablas d_*)
-- Cada tabla r_* replica la estructura de su equivalente d_* para que las
-- queries de reporte funcionen con mínimos cambios (solo renombrado de tabla).
-- DB separada de la desnormalizada (ver .env: DB_RESUMEN_*).
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS r_facturas (
    ...
    estado            VARCHAR(20)   NOT NULL COMMENT 'activa, pagada, vencida, anulada',
    created_at        DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at        DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at        DATETIME      NULL,
...
CREATE TABLE IF NOT EXISTS r_facturas_detalle (
    ...
    fecha_emision   DATE          NOT NULL,
    id_sucursal     INT           NOT NULL COMMENT 'FK original: sucursales.id',
    estado_factura  VARCHAR(20)   NOT NULL COMMENT 'activa, pagada, vencida, anulada',
    id_producto     INT           NOT NULL COMMENT 'FK original: productos.id',
    codigo_producto VARCHAR(50)   NOT NULL,
    nombre_producto VARCHAR(200)  NOT NULL,
